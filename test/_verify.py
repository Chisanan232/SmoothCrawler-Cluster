...
class VerifyMetaData:
    ...
    def group_state_info(
        ...
        _standby_id_checksum_list = list(
            map(lambda _crawler: _group_state.standby_id in _crawler, _group_state.current_backup))
        if len(_group_state.current_backup) != 0:
            assert True in _standby_id_checksum_list, \
    ...
    def group_state_current_section(
        ...
        if verify_runner is True:
            ...
            if fail_runner == 0:
                _runer_checksum_iter = map(
                    lambda _crawler: int(_crawler.split(index_sep_char)[-1]) <= runner,
                    _group_state.current_runner)
                assert (False not in list(_runer_checksum_iter),
        ...
        if verify_backup is True:
            ...
            if fail_runner == 0:
                _backup_checksum_iter = map(lambda _crawler: int(_crawler.split(index_sep_char)[-1]) > backup,
                                            _group_state.current_backup)
                assert (False not in list(_backup_checksum_iter),
    ...
    def one_task_info(
        ...
        if running_content_len is not None:
            assert (len(_task.running_content) == running_content_len,
                    f"The length of attribute 'running_content' should be {len(_task.running_content)}.")
        ...
        if in_progressing_id is not None:
            assert (_task.in_progressing_id == in_progressing_id,
                    f"The attribute 'in_progressing_id' should be '{in_progressing_id}'.")
        if running_status is not None:
            assert (_task.running_status == running_status,
        ...
        if running_result is not None:
            ...
            assert (_task.running_result == running_result,
    ...
    def one_heartbeat(self, heartbeat: Union[str, bytes, Heartbeat]) -> None:
        ...
        assert _diff_datetime.total_seconds() < \
               float(_heartbeat.update_time[:-1]) + float(_heartbeat.update_timeout[:-1]), ""
